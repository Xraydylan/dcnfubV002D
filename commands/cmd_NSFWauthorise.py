...
async def ex(args, message, client, invoke):
    ...
    if len(args) > 0:
        ...
        if args_out == "a big boy" or args_out == "A Big Boy":
            await assign_role("Im A Big Boy",message, client, message.channel, message.author)

        elif args_out == "a big girl" or args_out == "A Big Girl":
            await assign_role("Im A Big Girl",message, client, message.channel, message.author)
